...
module editor {
    export const defaultScripts: {[file: string]: string} = {
%s
    };
    export const defaultScriptNames: string[] = [
%s
    ];
}
`
...
func main() {
	...
	for name, script := range scripts {
		l := "        \"" + toStringLiteral(name) + "\": \"" + toStringLiteral(script) + "\""
		scriptLiterals = append(scriptLiterals, l)
	}
	nameLiterals := []string{}
	for _, name := range names {
		l := "        \"" + toStringLiteral(name) + "\""
		nameLiterals = append(nameLiterals, l)
	}

	fmt.Printf(tmpl, strings.Join(scriptLiterals, ",\n"), strings.Join(nameLiterals, ",\n"))
}
